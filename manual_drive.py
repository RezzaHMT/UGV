...
This script provides a KeyboardController class that uses the 'pynput' library
to listen for key presses and translate them into steering and throttle commands.
...
class KeyboardController:
    ...
    def __init__(self, max_steer_deg: int = 28, max_throttle: int = 100):
        """
        Initializes state variables and starts the keyboard listener.

        Args:
            max_steer_deg (int): Max steering angle from center. (max mechanical limit is approx. 30 degrees)
    ...
    def _on_press(self, key):
        ...
        try:
            ...
            if key == keyboard.Key.space:
                ...
                if self._throttle != 0: self._throttle = -abs(self._throttle)
    ...
    def steering(self) -> int:
        ...
        return int(self._steering * self._max_steer_deg)
...
def main():
    ...
    try:
        while kb.running:
            ...
            srv.set_angle(kb.steering)

            if kb.throttle == 0:
                mtr1.stop()
                mtr2.stop()
            else:
                mtr1.set_speed(kb.throttle)
                mtr2.set_speed(kb.throttle)
